...
```{r,echo=FALSE}
#Gráfico de barras de promedio de victorias de los equipos
GRAB3<- plot_ly(data=w_period_3,x=~TEAM, y=~w_prom, name = "Tercer Período", mode = "markers", color = "green", size = 10) %>% layout(title = "Media de victorias", xaxis = list(title="Equipos "), yaxis = list(title = "Victorias"))
GRAB3
```
...
```{r,echo=FALSE}
#Gráfico de caja y bigotes de equipos con mas y menos victorias en promedio
GRAC3<- plot_ly(data=df_period3,x=~TEAM, y=~W,name = "Tercer Período",  type = "box",mode = "markers", color = "green", size = 10) %>% layout(title = "Victorias del Tercer Período", xaxis = list(title="Equipos"), yaxis = list(title = "Victorias"))
GRAC3 
```
