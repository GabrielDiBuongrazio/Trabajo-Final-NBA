...
```{r, echo=FALSE}
GráficoDF <- plot_ly(team_by_period_w_prom, x=~TEAM, y=~w_prom,color =~season_by_period, marker = list(size =10), type = "scatter", mode= "markers") %>% layout(title = "Victorias desde 1994 hasta 2023", xaxis = list(title="Equipos"), yaxis = list(title = "Victorias en promedio"))
GráficoDF
```
...
```{r,echo=FALSE}
#Gráfico de barras de promedio de victorias de los equipos
GRAB3<- plot_ly(data=w_period_3_,x=~TEAM, y=~w_prom, name = "Tercer Período", marker = list(color = "blue", size= 10)) %>% layout(title = "Media de victorias", xaxis = list(title="Equipos "), yaxis = list(title = "Victorias"))
GRAB3
```
